...
function Seats() {
    ...
    if(checked){
        socket.emit("seatselecting",value,selectedScreen,[...isselectedSeat,value])
    }
    else{
...
if ((!(userSelected.find(seats =>seats===seatnumber)))&&(isholded.find(seats =>seats===seatnumber)||isbooked.find(seats =>seats===seatnumber))){
    return true
}
...
<div className='d-flex flex-row gap-3 gap-sm-5 flex-nowrap  mb-2'>
    ...
    {rows3.map((row_name,index)=>
            <div key={index} className=' d-flex flex-row flex-nowrap '>
               ...
               {seatsLeft.map((seat_no,index)=>
                   <div key={seat_no} className="btn-group" role="group" aria-label="Basic checkbox toggle button group">
                   <input type='checkbox'  className="btn-check  col-1"  id={`${row_name+seat_no}`} value={`${row_name+seat_no}`} 
                    onChange={handlechange} autoComplete="off" disabled={iSDisabled(row_name,seat_no)} />

                    <label  htmlFor={`${row_name+seat_no}`} >
                    <i className={"fa-solid fa-couch m-1 "+seatView(row_name,seat_no) }/>
   ...
   <div>
    ...
    {rows2.map((row_name,index)=>
            <div key={index} className=' d-flex flex-row flex-nowrap'>
               ...
               {seatsLeft.map((seat_no,index)=>
                  <div key={seat_no} className="btn-group" role="group" aria-label="Basic checkbox toggle button group">
                  <input type='checkbox'  className="btn-check  col-1" 
                   id={`${row_name+seat_no}`} value={`${row_name+seat_no}`}  onChange={handlechange} autoComplete="off" disabled={iSDisabled(row_name,seat_no)}  />
                   <label  htmlFor={`${row_name+seat_no}`} >
                   <i className={"fa-solid fa-couch m-1 "+seatView(row_name,seat_no) }/>
   ...
   <div>
    ...
    {rows1.map((row_name,index)=>
            <div key={index} className=' d-flex flex-row flex-nowrap'>
               ...
               {seatsLeft.map((seat_no,index)=>
                    <div key={seat_no} className="btn-group" role="group" aria-label="Basic checkbox toggle button group">
                    <input type='checkbox'  className="btn-check  col-1"  id={`${row_name+seat_no}`}
                     value={`${row_name+seat_no}`}  onChange={handlechange} autoComplete="off" disabled={iSDisabled(row_name,seat_no)}  />
                     <label  htmlFor={`${row_name+seat_no}`} >
                     <i className={"fa-solid fa-couch m-1 "+seatView(row_name,seat_no) }/>
   ...
   <div>
    ...
    <hr />
        ...
        <div>
    <span>
    AVAILABLE:<i className="fa-solid fa-couch text-success ms-2 me-4"/>
    WAITNG:<i className="fa-solid fa-couch text-warning ms-2 me-4"/>
    BOOKED:<i className="fa-solid fa-couch text-danger ms-2 me-4"/>
    </span>
</div>
